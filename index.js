...
app.get('/process', (req, res) => {
    ...
    .then(info => {
        ...
        if (audioFiles.includes(`${title}`)) {                                              // 오디오를 저장하는 폴더에 같은 영상이 있으면
            res.send(sendDownloadRedirect(`/audio/${title}.wav`, `${title}.wav`));          // 기존에 있던 파일로 다운로드
        } else {                                                                            // 아니면
            ytdl(`https://www.youtube.com/watch?v=${youtube_url}`, { filter: 'audioonly'})  // 파일 새로 다운받고 새로운 파일로 다운로드
            .pipe(fs.createWriteStream(`${__dirname}/static/audio/${title}.wav`));
            res.send(sendDownloadRedirect(`/audio/${title}.wav`, `${title}.wav`));
        }
    })
})
